...
export default function ContainersPage() {
  ...
  const fetchContainers = useCallback(async () => {
    setLoading(true);
    try {
      const data = await searchContainers(filters);
      setContainers(data);
    } catch {
  ...
  // Debounced search
  useEffect(() => {
    const timer = setTimeout(fetchContainers, 300);
      ...
      <div className="flex items-center justify-between">
        ...
        {isAdmin && (
          <button
            ...
            className="flex items-center gap-1.5 rounded-lg bg-brand-navy px-3 py-2 text-sm font-medium text-primary-foreground hover:opacity-90 transition-opacity"
          >
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="mt-1 text-xs text-muted-foreground/70">
            Спробуйте змінити фільтри або додайте нову тару
          </p>
        </div>
      ) : isDesktop ? (
